...
def tsmc_model_building(label):
    db = tsmc_main_dataset_loading(tsmc_file_dataset, tsmc_sheet_dataset)
    
    #identifying x & y variable
    db_x = db.iloc[:,col_x:]
    db_y = db[db.columns[pd.Series(db.columns).str.startswith('rt_')]].copy()
    ...
    cv_scores_pls = cross_validate(estimator=model_pls, X=X, y=y, cv=outer_cv, 
    ...
    pkl_filename = 'pls_model_'+str(label)'.pkl'
...
###############################################################################################################    
    ...
    np.random.seed(42)
    outer_cv = KFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores_rf = cross_validate(estimator=model_rf, X=X, y=y, cv=outer_cv, 
                              scoring=["neg_mean_squared_error", "neg_root_mean_squared_error", "neg_median_absolute_error", "r2"])
    model_rf.fit(X_train, y_train.values.ravel())

    pkl_filename = 'rf_model_'+str(label)'.pkl'
    with open(pkl_filename, 'wb') as file:
...
################################################################################################################
    ...
    np.random.seed(42)
    outer_cv = KFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores_xgb = cross_validate(estimator=model_xgb, X=X, y=y, cv=outer_cv, 
                              scoring=["neg_mean_squared_error", "neg_root_mean_squared_error", "neg_median_absolute_error", "r2"])
    model_xgb.fit(X_train, y_train.values.ravel())

    pkl_filename = 'xgb_model_'+str(label)'.pkl'
    with open(pkl_filename, 'wb') as file:
