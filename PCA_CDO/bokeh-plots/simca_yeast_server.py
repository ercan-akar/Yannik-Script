...
from bokeh.plotting import figure, output_file, show,  gridplot, curdoc
...
def plot_bokeh(y, X, average, upper, lower, DModX_upper, t2_upper, t2, t2_con, dx, dx_con, file_in):
    ...
    # stupid hack to ensure the plotting
    upper[0] = None
    lower[0] = None
    average[0] = None
    DModX_upper[0] = None
    ...
    s3 = ColumnDataSource(X[0:timestep])     
    ...
    p = figure(plot_width=600, plot_height=400, title = "Principal Component 1",
                   x_axis_label = "Time [h]",
...
# 
    p_series = figure(plot_width=600, plot_height=330, title = colnames[0],
                   x_axis_label = "Time [h]",
                   tools =['pan,box_select','tap','reset'])
...
##select.on_change('value', update_plot)
    ...
    select = Select(value=colnames[0], title="Select Parameter", options=colnames)
#cols = colnames[1]
#
#
#
    ...
    def update():
        ...
        X['y'] = X[p_series.title.text]
        Y = X[timestep-1:timestep]
        #Y['y'] = X[timestep-1:timestep, p_series.title.text]
        s4 = ColumnDataSource(Y)
        # s4[y] = s3[y??]
        s3.stream(s4.data)
        
        print(p_series.title.text)
...
#            #t_con[i,j] = a[i,j]**2/DSD[j,j]
    ...
    if (is_pls):
        ...
        if (variant == 4):
            ...
            Gamma = la.sqrtm(RTR + np.diag(zero_row))
...
def get_DModX(X, pls_or_pca, scaler): 
    # Select "Absolute" and deselect "Weighted by the modelling Power" 
    # in SIMCA to get same results
    # SIMCA states, that the DMODX of observations included in the model
    # are multiplied with a scaling factor > 1. I could not spot a difference.

    # the contributions of SIMCA have some scaling compared to these results
    ...
    if is_pls:
        DModX_con = (X_scaled - np.dot(np.dot(P,R.T), X_scaled.T).T)/np.sqrt((I-A))
    if (not is_pls):
        DModX_con = X_scaled - np.dot(T,P)
    
    DModX = np.linalg.norm(DModX_con, axis = 1)
...
if (len(sys.argv) > 1):
    if (sys.argv[1] == 'good'):
        msk = (yeast_data.iloc[:,1] == 'Ba')
        xbad_u = np.array(pd.DataFrame(yeast_data[msk]).iloc[:,3:])
    else: # standat: show bad batch
...
for n in range(ncomp):
    # we need the empirical standartdeviation
    std_array[:,n] = X_pls_pca[:,n].reshape(Xdata.shape[0],-1, order='F').std(axis=1, ddof = 1)
    avg_array[:,n] = X_pls_pca[:,n].reshape(Xdata.shape[0],-1, order='F').mean(axis=1)


# calc for DModX
DModX, DModX_con = get_DModX(xgood.copy(), pls, scaler)
DModXstd = DModX.reshape(Xdata.shape[0],-1, order='F').std(axis=1, ddof = 1)
...
t2, t2_con = get_t2(xbad_u.copy(), pls, scaler, X, variant = 0)
t2_con[0,:]
...
#plt.plot(upper); plt.plot(lower); plt.plot(xbad[:,pr_comp])
#plt.plot(DModXmean);plt.plot(DModX_upper);
...
#plot_bokeh(xbad[0:timestep,pr_comp], X_plot.iloc[0:timestep,:], avg[0:timestep], 
#           upper[0:timestep], lower[0:timestep], DModX_upper[0:timestep], 
#            t2_upper[0:timestep], t2[0:timestep], t2_con[0:timestep], DModX[0:timestep], DModX_con[0:timestep], file_pls)
